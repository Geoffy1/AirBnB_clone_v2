...
def html_if_int(n):
    """display html pg only if int given
...
def html_odd_or_even(n):
    """display html pg only if int given
       place given int into html template
       substitute txt to display if int is odd or even
    """
...
def html_if_stateID(id):
    """display html pg; customize heading with state.name
...
def html_filters():
    """display html pg with working city/state filters & amenities
...
def html_all_filters():
    """display html pg w/ working city/state filters & amenities/properties
       runs with web static css files
    """
    state_objs = [s for s in storage.all("State").values()]
    amenity_objs = [a for a in storage.all("Amenity").values()]
